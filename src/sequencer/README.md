...
- надо продумать как и где ресетать все послед
- продумать получше сценарий когда надо отрендерить финальное состояние анимированных сущностей (initially completed)
  - там duration/delay по сути уже должен быть нулевым (тут опять возникает идея разделить концепты blueprint и execution)

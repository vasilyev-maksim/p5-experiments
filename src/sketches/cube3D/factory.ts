...
export const factory: ISketchFactory<Params> = createSketch<Params>(
  ({ p, getProp, getTime }) => {
    ...
    return {
      setup: () => {
        // const w = getProp("canvasWidth");
        // const h = getProp("canvasHeight");
      ...
      draw: () => {
        function project(v: p5.Vector) {
          return p.createVector(v.x / v.z, v.y / v.z);
        }

        function shiftZ(v: p5.Vector, delta: number) {
          return p.createVector(v.x, v.y, v.z + delta);
        }

        function screen(v: p5.Vector) {
          const w = getProp("canvasWidth");
          const h = getProp("canvasHeight");
          const min = Math.min(w, h);
        ...
        function rotateXZ(v: p5.Vector, angle: number) {
          // return v;
          const c = p.cos(angle),
            s = p.sin(angle),
            x = v.x * c - v.z * s, // Исправлено: минус вместо плюс
            z = v.x * s + v.z * c; // Исправлено: плюс вместо минус
          const newv = p.createVector(x, v.y, z);
          // console.log(p.cos(angle), Math.cos(angle));
          // console.log(v, newv, angle);
          return newv;
        }
        ...
        return () => {
          ...
          // const w = getProp("canvasWidth");
          // const h = getProp("canvasHeight");
          const angleDelta = time * 0.005;
          const zoom = getProp("ZOOM");
          // const min = Math.min(w, h);
          // p.scale(min, min);
          // p.translate(w / 2, h / 2);
          // p.scale(1, -1);
          ...
          // vs.forEach((v) => {
          //   const { x, y } = screen(
          //     project(shiftZ(rotateXZ(v, angleDelta), 1)),
          //   );
          //   // const { x, y } = screen(project(v));
          //   p.circle(x, y, 5);
          // });
          ...
          es.forEach((arr) => {
            arr.forEach((el, i) => {
              const v1 = screen(
                project(shiftZ(rotateXZ(vs[el], angleDelta), zoom)),
              );
              const v2 = screen(
                project(
                  shiftZ(
                    rotateXZ(vs[arr[(i + 1) % arr.length]], angleDelta),
                    zoom,
                  ),
                ),
              );
...
//   { x: 0.322171, y: -0.510414, z: 0.081591 },
//   { x: 0.322171, y: -0.244774, z: 0.081591 },
// ].map((x) => p.createVector(x.x, x.y, x.z));

...
export const factory = createSketch<Controls>(
  ({
    ...
    function getNodes(): [number, number][] {
      ...
      return range(POLYGONS_COUNT).map((i) => {
        return [
          i * d,
          i *
            getParam("COIL_FACTOR") *
            (coilSpeed === 0 ? 1 : p.sin(time / coilSpeed)) +
            time * getParam("ROTATION_SPEED"),
        ];
      });
    }
    ...
    function drawPolygon([d, angle]: [number, number]) {
      ...
      const adelta = 360 / getParam("POLYGON_N");
      ...
      draw: () => () => {
        ...
        nodes.forEach((node, i, arr) => {
          const fillColor = p.lerpColor(
            ...
            colorB,
            p.sin(
              getTime() * getParam("COLOR_CHANGE_SPEED") +
                (i / arr.length) * 360 * 4,
            ),
          );
          ...
          if (getParam("POLYGON_N") === controls.POLYGON_N.max) {
            drawCircle(node);
          } else {
            drawPolygon(node);
          }
